...
namespace DWSIM.Engine.SmokeTests
{
    ...
    public class RigorousColumnTests
    {
        ...
        // 300-iteration budget nor the wide-boiling retry converges it. At a quarter step it
        // converges in 47 sweeps, at a tenth in 69. The solver has to find that on its own.
        [Test]
        public void TheColdNaphthaStabiliserConvergesOnTheRelaxedRetry()
        {
            ...
            Assert.That(column.TemperatureRelaxation, Is.EqualTo(0.5), "the file carries no setting; the default step is the shipped one");
        ...
        // The bubble-point solvers move each stage temperature half way towards its new
        // bubble point on every sweep. TemperatureRelaxation makes that fraction a column
        // setting, default the historical 0.5, and it has to survive a save and a load
        // like any other setting.
        [Test]
        public void TemperatureRelaxationIsAColumnSettingThatRoundTrips()
        {
            var flowsheet = Load("WideBoilingStripper.dwxmz");
            var column = flowsheet.SimulationObjects.Values.OfType<DistillationColumn>().Single();

            Assert.That(column.TemperatureRelaxation, Is.EqualTo(0.5),
                        "a file saved before the setting existed keeps the historical half step");

            column.TemperatureRelaxation = 0.1;
            var saved = column.SaveData();
            column.TemperatureRelaxation = 0.5;
            column.LoadData(saved);

            Assert.That(column.TemperatureRelaxation, Is.EqualTo(0.1));
        }
