...
Namespace UnitOperations.Auxiliary.SepOps.SolvingMethods
    ...
    <System.Serializable()> Public Class WangHenkeMethod
        ...
        ' The temperature step in force for the current solve: the column's TemperatureRelaxation to
        ...
        Public Function Solve(ByVal rc As Column, ByVal nc As Integer, ByVal ns As Integer, ByVal maxits As Integer,
            ...
            _trelax = 0.5
            If rc.TemperatureRelaxation > 0.0 AndAlso rc.TemperatureRelaxation <= 1.0 Then _trelax = rc.TemperatureRelaxation
        ...
        ''' <summary>
        ''' One bubble-point solve with its fallbacks. The column's temperature step first; when the iteration
        ''' does not converge, the step halved down to 0.1 (0.5, 0.25, 0.1) and the solve repeated from the same
        ''' start; when that fails too, the wide-boiling path (Mode 1) as before. A column that converges at its
        ''' own step never reaches the fallbacks, so nothing that solves today runs differently. The step that
        ...
        Public Function Solve_Internal(rc As Column, nc As Integer, ns As Integer, maxits As Integer,
            ...
            ' solve (the column's TemperatureRelaxation, lowered by SolveWithFallbacks on a failure).
